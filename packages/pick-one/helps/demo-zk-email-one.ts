...
import { generateInputs } from "./generate-input-one";
...
if (BUILD_DIR == null) {
  BUILD_DIR = path.join(__dirname, "../build-one");
  log("No BUILD_DIR provided, using /build-one");
}
if (OUTPUT_DIR == null) {
  OUTPUT_DIR = path.join(__dirname, "../proofs-one");
  log("No OUTPUT_DIR provided, using /proofs-one");
}
