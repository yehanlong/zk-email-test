...
import { generateInputs } from "./generate-input-from";
...
if (CIRCUIT_NAME == null) {
  CIRCUIT_NAME = "demo-zk-email-from";
  log("No CIRCUIT_NAME provided, using demo-zk-email-from");
}
if (BUILD_DIR == null) {
  BUILD_DIR = path.join(__dirname, "../build-from");
  log("No BUILD_DIR provided, using /build-from");
}
if (OUTPUT_DIR == null) {
  OUTPUT_DIR = path.join(__dirname, "../proofs-from");
  log("No OUTPUT_DIR provided, using /proofs-from");
}
...
async function generate() {
  ...
  const input = await generateInputs(rawEmail);

  const circuitInputs = { msg: input.emailHeader, fromEmailIndex : input.fromEmailIndex};
